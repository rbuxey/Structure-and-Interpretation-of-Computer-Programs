...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 5
        exp_rest = expressions.rest
        val = scheme_eval(exp_rest.first, env)
        ...
        _lambda = do_lambda_form(Pair(formals, body), env)
        env.define(name, _lambda)
        return name
        # END PROBLEM 9
    else:
...
def make_let_frame(bindings, env):
    ...
    while bindings != nil:
        ...
        val = scheme_eval(first.rest.first, env)
...
def do_define_macro(expressions, env):
    ...
    validate_form(expressions, 2)
    target = expressions.first
    validate_formals(target)
    validate_form(target, 0)
    name = target.first
    formals = target.rest
    body = expressions.rest
